...
import ove_ext.dain.correlation as my_lib
...
class CorrelationFunction(Function):
    @staticmethod
    def forward(
            ctx, input1, input2, pad_size=3, kernel_size=3,
...
def conv(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1):
    return Sequential(
        nn.Conv2d(in_planes, out_planes, kernel_size=(kernel_size, kernel_size), stride=(stride, stride),
                  padding=padding, dilation=(dilation, dilation), bias=True),
        nn.LeakyReLU(0.1, True))


def predict_flow(in_planes):
    return nn.Conv2d(in_planes, 2, kernel_size=(3, 3), stride=(1, 1), padding=1, bias=True)
...
class PWCDCNet(nn.Module):
    def __init__(self, width, height):
        ...
        xx = torch.arange(0, W_MAX, device='cuda').view(1, -1)
        yy = torch.arange(0, H_MAX, device='cuda').view(-1, 1)
