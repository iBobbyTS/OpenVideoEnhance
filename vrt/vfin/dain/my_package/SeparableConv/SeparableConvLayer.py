...
class SeparableConvLayer(Function):
    ...
    def backward(self, gradoutput):
        ...
        if self.input1.is_cuda:
            ...
            err = my_lib.SeparableConvLayer_gpu_backward(self.input1, self.input2, self.input3, gradoutput, gradinput1,
                                                         gradinput2, gradinput3)
            if err != 0:
                print(err)
            ...
            # print("CPU backward")
            # print(gradoutput)
            err = my_lib.SeparableConvLayer_cpu_backward(self.input1, self.input2, self.input3, gradoutput, gradinput1,
                                                         gradinput2, gradinput3)
            # print(err)
            if err != 0:
                print(err)
