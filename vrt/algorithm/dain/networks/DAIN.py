...
class DAIN(torch.nn.Module):
    def __init__(self,
        ...
        assert (
                timestep == 0.5)  # TODO: or else the WeigtedFlowProjection should also be revised... Really Tedious work.
    ...
    def forward(self, input):
        ...
        cur_output, ref0, ref2 = self.FilterInterpolate(cur_input_0, cur_input_2, cur_offset_output, cur_filter_output,
                                                        self.filter_size ** 2)
    ...
    def forward_flownets(self, model, input, time_offsets=None):
        ...
        temps = [nn.Upsample(scale_factor=4, mode='bilinear')(temp) for temp in
